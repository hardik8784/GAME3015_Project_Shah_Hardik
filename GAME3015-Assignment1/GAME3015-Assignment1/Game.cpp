...
bool Game::Initialize()
{
	...
	mMainWndCaption = L"GAME3015_Project_Shah_Hardik";
...
void Game::LoadTextures()
{
	...
	//Selector Tex
	auto ControlsTex = std::make_unique<Texture>();
	ControlsTex->Name = "ControlsTex";
	ControlsTex->Filename = L"../../Textures/Instructions.dds";
	ThrowIfFailed(DirectX::CreateDDSTextureFromFile12(md3dDevice.Get(),
		mCommandList.Get(), ControlsTex->Filename.c_str(),
		ControlsTex->Resource, ControlsTex->UploadHeap));

	mTextures[ControlsTex->Name] = std::move(ControlsTex);
...
void Game::BuildDescriptorHeaps()
{
	...
	auto ControlsTex = mTextures["ControlsTex"]->Resource;
	...
	//Controls Descriptor
	hDescriptor.Offset(1, mCbvSrvDescriptorSize);
	srvDesc.Format = ControlsTex->GetDesc().Format;
	md3dDevice->CreateShaderResourceView(ControlsTex.Get(), &srvDesc, hDescriptor);
...
void Game::BuildMaterials()
{
	...
	//auto PauseLabel = std::make_unique<Material>();
	//PauseLabel->Name = "Pause";
	//PauseLabel->MatCBIndex = 5;
	//PauseLabel->DiffuseSrvHeapIndex = 5;
	//PauseLabel->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	//PauseLabel->FresnelR0 = XMFLOAT3(0.05f, 0.05f, 0.05f);
	//PauseLabel->Roughness = 0.2f;
	//mMaterials["Pause"] = std::move(PauseLabel);
	...
	auto Controls = std::make_unique<Material>();
	Controls->Name = "Controls";
	Controls->MatCBIndex = 7;
	Controls->DiffuseSrvHeapIndex = 7;
	Controls->DiffuseAlbedo = XMFLOAT4(1.0f, 1.0f, 1.0f, 1.0f);
	Controls->Roughness = 0.2f;
	mMaterials["Controls"] = std::move(Controls);
